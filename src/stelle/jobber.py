...
peclet=5#35/sigma         #peclet number (self propulsion strength) in sigma/s
...
Dr=1.0#0.8                 #if brownian=1, defines the rotational diffusion coefficient Dr in rad^2/s
...
r_ints=[4,5,6,7,8,9,10,12,14,16,18,20]#50/sigma          #calculation of effective interaction between stars in sigma. If r_int>0, this automatically sets mol=2, gh=0, rconf=0, fixes the core positions and outputs their forces
    ...
    for fval in fs:
        for rint in r_ints:
            details = {"n_beads": n_beads, "n_mol": n_mol,"functionality": fval,
                "r_core": r_core, "peclet":peclet, "r_conf":r_conf, "r_bond":r_bond, "r_cbond":r_cbond, "r_int":rint, "seed_start": seed, "ghost":gh, "brownian":brownian, "Dr":Dr}
