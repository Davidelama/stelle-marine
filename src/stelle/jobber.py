...
def job_maker(details):
    ...
    runtime = 2e9#1e10#5e6# #cannot be larger than 2e9!
    dumptime = 1e6#1e8#2e4#
    dumptime_msd = 2e7#5e4#
    dumptime_angle = 1e4#2e1#
    restime = 1e8
...
n_beads=[15]#[7,9,11,13]#[5,7,9,11,13,15]    #number of beads per arm
...
r_ints=[4,5,6,7,8,9,10,12,14,16,18,20]          #calculation of effective interaction between stars in sigma. If r_int>0, this automatically sets mol=2, gh=0, rconf=0, fixes the core positions and outputs their forces
...
d_pass=[0]#[0.01, 0.03,0.05,0.07,0.1,0.15,0.2]                #if >0, introduce passive particles to be captured by the stars
