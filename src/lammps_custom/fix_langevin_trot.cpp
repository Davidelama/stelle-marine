...
FixLangevinTrot::FixLangevinTrot(LAMMPS *lmp, int narg, char **arg) :
...
{
  if (narg < 7) error->all(FLERR, "Illegal fix langevin/trot command");
  ...
  if (t_period <= 0.0) error->all(FLERR, "Fix langevin/trot period must be > 0.0");
  if (seed <= 0) error->all(FLERR, "Illegal fix langevin/trot command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "angmom") == 0) {
      if (iarg + 2 > narg) error->all(FLERR, "Illegal fix langevin/trot command");
      ...
      if (strcmp(arg[iarg + 1], "no") == 0) {
        ...
        error->all(FLERR, "Illegal fix langevin/trot command");
      iarg += 2;
      ...
      if (iarg + 3 > narg) error->all(FLERR, "Illegal fix langevin/trot command");
      int itype = utils::inumeric(FLERR, arg[iarg + 1], false, lmp);
      double scale = utils::numeric(FLERR, arg[iarg + 2], false, lmp);
      if (itype <= 0 || itype > atom->ntypes) error->all(FLERR, "Illegal fix langevin/trot command");
      ratio[itype] = scale;
      iarg += 3;
      ...
      if (iarg + 4 > narg) error->all(FLERR, "Illegal fix langevin/trot trot command");
      t_rot_flag = 1;
      
      if (utils::strmatch(arg[iarg + 1], "^v_")) {
        tstr_rot = utils::strdup(arg[iarg + 1] + 2);
      } else {
        t_start_rot = utils::numeric(FLERR, arg[iarg + 1], false, lmp);
        t_target_rot = t_start_rot;
        tstyle_rot = CONSTANT;
      }
      
      t_stop_rot = utils::numeric(FLERR, arg[iarg + 2], false, lmp);
      t_period_rot = utils::numeric(FLERR, arg[iarg + 3], false, lmp);
      
      if (t_period_rot <= 0.0) 
        error->all(FLERR, "Fix langevin/trot trot period must be > 0.0");
      iarg += 4;
    } else
      error->all(FLERR, "Illegal fix langevin/trot command");
  }
  ...
  if (gjfflag) {
    ...
    FixLangevinTrot::grow_arrays(atom->nmax);
...
FixLangevinTrot::~FixLangevinTrot()
{
  if (copymode) return;
...
void FixLangevinTrot::init()
{
  ...
  if (gjfflag) {
    if (t_period * 2 == update->dt)
      error->all(FLERR, "Fix langevin/trot gjf cannot have t_period equal to dt/2");
    ...
    if (flag) error->all(FLERR, "Fix langevin/trot gjf should come before fix nve");
  }

  if (oflag && !atom->omega_flag)
    error->all(FLERR, "Fix langevin/trot omega requires atom attribute omega");
  if (oflag && !atom->radius_flag)
    error->all(FLERR, "Fix langevin/trot omega requires atom attribute radius");
  if ((ascale != 0.0) && !atom->ellipsoid_flag)
    error->all(FLERR, "Fix langevin/trot angmom requires atom style ellipsoid");

  // check variable

  if (tstr) {
    tvar = input->variable->find(tstr);
    if (tvar < 0) error->all(FLERR, "Variable name {} for fix langevin/trot does not exist", tstr);
    if (input->variable->equalstyle(tvar))
      tstyle = EQUAL;
    else if (input->variable->atomstyle(tvar))
      tstyle = ATOM;
    else
      error->all(FLERR, "Variable {} for fix langevin/trot is invalid style", tstr);
  }

  if (tstr_rot) {
    tvar_rot = input->variable->find(tstr_rot);
    if (tvar_rot < 0) 
      error->all(FLERR, "Variable name {} for fix langevin/trot trot does not exist", tstr_rot);
    if (input->variable->equalstyle(tvar_rot))
      tstyle_rot = EQUAL;
    else if (input->variable->atomstyle(tvar_rot))
      tstyle_rot = ATOM;
    else
      error->all(FLERR, "Variable {} for fix langevin/trot trot is invalid style", tstr_rot);
  }
  ...
  if (oflag) {
    ...
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit)
        if (radius[i] == 0.0) error->one(FLERR, "Fix langevin/trot omega requires extended particles");
  }

  if (ascale != 0.0) {
    avec = dynamic_cast<AtomVecEllipsoid *>(atom->style_match("ellipsoid"));
    if (!avec) error->all(FLERR, "Fix langevin/trot angmom requires atom style ellipsoid");
    ...
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit)
        if (ellipsoid[i] < 0) error->one(FLERR, "Fix langevin/trot angmom requires extended particles");
  }
  ...
  if (utils::strmatch(update->integrate_style, "^respa")) {
    ...
    if (gjfflag) error->all(FLERR, "Fix langevin/trot gjf and run style respa are not compatible");
  }
...
void FixLangevinTrot::setup(int vflag)
{
  if (gjfflag) {
    double dtfm;
    double dt = update->dt;
...
void FixLangevinTrot::initial_integrate(int /* vflag */)
{
  double **v = atom->v;
  double **f = atom->f;
  int *mask = atom->mask;
...
void FixLangevinTrot::post_force(int /*vflag*/)
{
  double *rmass = atom->rmass;
...
void FixLangevinTrot::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixLangevinTrot::post_force_templated()
{
...
void FixLangevinTrot::compute_target()
{
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
  ...
  if (tstyle == CONSTANT) {
    ...
    if (tstyle == EQUAL) {
      ...
      if (t_target < 0.0)
        error->one(FLERR, "Fix langevin/trot variable returned negative temperature");
      tsqrt = sqrt(t_target);
    } else {
      if (atom->nmax > maxatom2) {
        maxatom2 = atom->nmax;
        memory->destroy(tforce);
        memory->create(tforce,maxatom2,"langevin/trot:tforce");
      }
...
/* ----------------------------------------------------------------------
   set current rotational t_target_rot and tsqrt_rot
------------------------------------------------------------------------- */

void FixLangevinTrot::compute_target_rot()
{
  int *mask = atom->mask;
  int nlocal = atom->nlocal;

  // If no separate rotational temp specified, use translational
  if (!t_rot_flag) {
    t_target_rot = t_target;
    tsqrt_rot = tsqrt;
    return;
  }
  ...
  if (tstyle_rot == CONSTANT) {
    t_target_rot = t_start_rot + delta * (t_stop_rot - t_start_rot);
    tsqrt_rot = sqrt(t_target_rot);
  } else {
    modify->clearstep_compute();
    if (tstyle_rot == EQUAL) {
      t_target_rot = input->variable->compute_equal(tvar_rot);
      if (t_target_rot < 0.0)
        error->one(FLERR, "Fix langevin/trot trot variable returned negative temperature");
      tsqrt_rot = sqrt(t_target_rot);
    } else {
      if (atom->nmax > maxatom3) {
        maxatom3 = atom->nmax;
        memory->destroy(tforce_rot);
        memory->create(tforce_rot, maxatom3, "langevin/trot:tforce_rot");
      }
      input->variable->compute_atom(tvar_rot, igroup, tforce_rot, 1, 0);
      for (int i = 0; i < nlocal; i++)
        if (mask[i] & groupbit)
          if (tforce_rot[i] < 0.0)
            error->one(FLERR, "Fix langevin/trot trot variable returned negative temperature");
...
void FixLangevinTrot::omega_thermostat()
{
  double gamma1, gamma2;
  ...
  // Compute rotational target temperature
  compute_target_rot();
  ...
  for (int i = 0; i < nlocal; i++) {
    if ((mask[i] & groupbit) && (radius[i] > 0.0)) {
      ...
      // Use per-atom rotational temp if ATOM style, otherwise use global
      if (tstyle_rot == ATOM) 
        tsqrt_use = sqrt(tforce_rot[i]);
      else 
        tsqrt_use = tsqrt_rot;
      
      gamma1 = -tendivthree * inertiaone / t_period_rot / ftm2v;
      gamma2 = sqrt(inertiaone) * sqrt(80.0 * boltz / t_period_rot / dt / mvv2e) / ftm2v;
      gamma1 *= 1.0 / ratio[type[i]];
      gamma2 *= 1.0 / sqrt(ratio[type[i]]) * tsqrt_use;
...
void FixLangevinTrot::angmom_thermostat()
{
  double gamma1, gamma2;
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      gamma1 = -ascale / t_period_rot / ftm2v;
      gamma2 = sqrt(ascale * 24.0 * boltz / t_period_rot / dt / mvv2e) / ftm2v;
      gamma1 *= 1.0 / ratio[type[i]];
      gamma2 *= 1.0 / sqrt(ratio[type[i]]) * tsqrt_use;
...
void FixLangevinTrot::end_of_step()
{
  double **v = atom->v;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixLangevinTrot::reset_target(double t_new)
{
...
int FixLangevinTrot::modify_param(int narg, char **arg)
{
...
double FixLangevinTrot::compute_scalar()
{
...
void *FixLangevinTrot::extract(const char *str, int &dim)
{
  dim = 0;
...
double FixLangevinTrot::memory_usage()
{
  double bytes = 0.0;
  ...
  if (tforce_rot) bytes += (double) atom->nmax * sizeof(double);
  return bytes;
}
...
void FixLangevinTrot::grow_arrays(int nmax)
{
  memory->grow(franprev, nmax, 3, "fix_langevin_trot:franprev");
  memory->grow(lv, nmax, 3, "fix_langevin_trot:lv");
}
...
void FixLangevinTrot::copy_arrays(int i, int j, int /*delflag*/)
{
...
int FixLangevinTrot::pack_exchange(int i, double *buf)
{
  int n = 0;
  buf[n++] = franprev[i][0];
  buf[n++] = franprev[i][1];
...
int FixLangevinTrot::unpack_exchange(int nlocal, double *buf)
{
  int n = 0;
