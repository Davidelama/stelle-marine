...
#35 1pass6b9
#13*5 1act6b9
#18*5 act3arm11beads0.03dpass
#1161/1350 act6arm11beads0.03dpass
#20 pass3arm11beads0.03dpass
#50 pass6arm11beads0.03dpass

chosen_snapshot = 20#24*5#2mpass53*5#2mact25*5#
molcol=True
armcol=False
ccol=True
...
if details['r_conf'] != 0:
    ...
    if molcol:
        for f in range(0,details["functionality"]*details["n_mol"]):
            ax.plot(data.x[data.p_idx == f], data.y[data.p_idx == f], color=cols[f//details["functionality"]],lw=2)
        ax.scatter(data.x[data.type == 2], data.y[data.type == 2], s=s2, edgecolors="k", color=cols[data.p_idx[data.type==2].values.astype(int)//details["functionality"]])
        ax.scatter(data.x[data.type==3],data.y[data.type==3],s=s,edgecolors= cols[data.p_idx[data.type==3].values.astype(int)//details["functionality"]],color="w",lw=2)
        ...
        ax.scatter(data.x[data.type == 2], data.y[data.type == 2], s=s2, edgecolors="k", color=armcols[data.p_idx[data.type==2].values.astype(int)])
        ax.scatter(data.x[data.type==3],data.y[data.type==3],s=s,edgecolors= armcols[data.p_idx[data.type==3].values.astype(int)],color="w",lw=2)
        ax.scatter(data.x[data.type == 3] + np.cos(data.theta[data.type == 3]) * diam * .25,
                data.y[data.type == 3] + np.sin(data.theta[data.type == 3]) * diam * .25, s=s * .09, color="k")
    else:
        for f in range(0,details["functionality"]*details["n_mol"]):
...
fig.savefig("../../data/03_analyzed/snapshots/"+IO.get_name(details)+f"_s{chosen_snapshot}.png", bbox_inches="tight",dpi=300)
plt.show()
