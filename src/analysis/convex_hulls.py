...
for i, t in enumerate(timesteps[initial_skip:initial_skip+max_steps:nskip]):
    ...
    intersections=[]
    for j in data_p.mol_id.unique():
        hval=ConvexHull(points[data_p.mol_id==j,:])
        hull_pts=points[data_p.mol_id==j,:][hval.vertices]
    ...
    area_its_ratio=big_its.area/big_poly.area
    ...
    ax.text(0.75, .95, r"$A_i=$"+f"{area_its_ratio:.3f}", color="k", transform=ax.transAxes, fontsize=20)
    ax.text(0.65, .01, r"$c_v=$" + f"{rclose:.5f}", color="k", transform=ax.transAxes, fontsize=20)
    ax.text(0.01, .01, r"$c_v/A_i=$" + f"{rclose/area_its_ratio:.3f}", color="k", transform=ax.transAxes, fontsize=20)
...
animation.save("../../data/03_analyzed/hulls/"+IO.get_name(details)+".mp4")
